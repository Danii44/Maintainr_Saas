...
function AnalyticsPanel({ role }: { role: PortalRole }) {
  ...
  const cards = [
    { label: t("Open work", "الأعمال المفتوحة"), value: metrics?.openTickets ?? 0, tone: "maintainr-operation-tone-primary" },
    { label: t("Scheduled visits", "الزيارات المجدولة"), value: metrics?.scheduledVisits ?? 0, tone: "maintainr-operation-tone-secondary" },
    { label: t("Urgent attention", "حالات عاجلة"), value: metrics?.urgentTickets ?? 0, tone: "maintainr-operation-tone-warning" },
    { label: t("Open inquiries", "استفسارات مفتوحة"), value: metrics?.openInquiries ?? 0, tone: "maintainr-operation-tone-accent" },
  ];
  return <>
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map((card) => <Card key={card.label} className="maintainr-operation-card maintainr-card-enter maintainr-card-affordance" data-card-tip={t("Live workspace count. Use the dedicated dashboard section for full detail.", "عدد مباشر لمساحة العمل. استخدم قسم لوحة المعلومات للتفاصيل الكاملة.")}><CardContent className="relative p-5"><div className={`maintainr-operation-card-accent ${card.tone}`}/><div className="maintainr-operation-label">{card.label}</div><div className="maintainr-operation-value">{summary.isLoading ? "—" : card.value}</div><progress className={`maintainr-operation-progress ${card.tone}`} value={Math.min(100, Math.max(14, Number(card.value) * 22))} max={100} aria-label={card.label}/></CardContent></Card>)}</div>
    <div className="mt-6 grid gap-6 xl:grid-cols-[.85fr_1.5fr_.85fr]">
      <Card className="maintainr-operation-card maintainr-card-enter maintainr-card-affordance" data-card-tip={t("The percentage of visible requests that have reached resolution.", "النسبة المئوية للطلبات الظاهرة التي وصلت إلى الحل.")}><CardHeader><CardTitle className="maintainr-operation-card-title">{t("Resolution progress", "تقدم الحلول")}</CardTitle></CardHeader><CardContent className="flex flex-col items-center pb-7"><div className="maintainr-resolution-meter"><span className="maintainr-operation-value">{completion}%</span><span>{t("resolved", "محلول")}</span></div><progress className="maintainr-resolution-progress" value={completion} max={100} aria-label={t("Resolution progress", "تقدم الحلول")}/><p className="maintainr-operation-description">{t("Based on requests visible to this role.", "بناءً على الطلبات الظاهرة لهذا الدور.")}</p></CardContent></Card>
      <Card className="maintainr-operation-card maintainr-card-enter maintainr-card-affordance" data-card-tip={t("A live comparison of requests, visits, urgent work, questions, and resolutions.", "مقارنة مباشرة للطلبات والزيارات والعمل العاجل والأسئلة والحلول.")}><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle className="maintainr-operation-card-title">{t("Workload trend", "اتجاه عبء العمل")}</CardTitle><p className="maintainr-operation-description">{t("Request movement across the current workspace.", "حركة الطلبات عبر مساحة العمل الحالية.")}</p></div><span className="maintainr-operation-live-badge">{t("Live", "مباشر")}</span></CardHeader><CardContent className="pb-7"><div className="maintainr-operation-bars">{[metrics?.openTickets ?? 0, metrics?.scheduledVisits ?? 0, metrics?.urgentTickets ?? 0, metrics?.openInquiries ?? 0, metrics?.resolvedTickets ?? 0].map((value, index) => <div key={index} className="maintainr-operation-bar"><progress value={Math.max(16, Math.min(150, Number(value) * 28 + 28))} max={150} aria-label={[t("Open", "مفتوح"), t("Visits", "زيارات"), t("Urgent", "عاجل"), t("Questions", "أسئلة"), t("Resolved", "محلول")][index]}/><span>{[t("Open", "مفتوح"), t("Visits", "زيارات"), t("Urgent", "عاجل"), t("Questions", "أسئلة"), t("Resolved", "محلول")][index]}</span></div>)}</div></CardContent></Card>
      <Card className="maintainr-next-focus maintainr-card-enter maintainr-card-affordance" data-card-tip={t("The clearest next action based on current workspace activity.", "الإجراء التالي الأكثر وضوحاً بناءً على نشاط مساحة العمل الحالي.")}><CardContent className="p-6"><div className="maintainr-next-focus-eyebrow">{t("Next focus", "التركيز التالي")}</div><div className="maintainr-next-focus-title">{metrics?.urgentTickets ? t("Review urgent maintenance first.", "راجع الصيانة العاجلة أولاً.") : t("Your active work is under control.", "أعمالك النشطة تحت السيطرة.")}</div><p className="maintainr-next-focus-description">{t("Use the calendar to confirm visits and messages to keep everyone informed.", "استخدم التقويم لتأكيد الزيارات والرسائل لإبقاء الجميع على اطلاع.")}</p></CardContent></Card>
    </div>
    {role === "PROPERTY_MANAGER" && <div className="mt-6 grid gap-6 xl:grid-cols-3"><Card className="maintainr-oversight-card maintainr-card-enter"><CardHeader><CardTitle className="maintainr-oversight-title">{t("Evidence review", "مراجعة الأدلة")}</CardTitle></CardHeader><CardContent className="space-y-3">{(oversight.data?.evidence ?? []).slice(0, 3).map((item) => <a key={item.id} href={item.url} target="_blank" rel="noreferrer" className="maintainr-oversight-item">{item.fileName}<span className="maintainr-oversight-item-meta mt-1">{item.purpose.replace("_", " ")} · {new Date(item.createdAt).toLocaleDateString()}</span></a>)}{!oversight.isLoading && !(oversight.data?.evidence ?? []).length && <p className="maintainr-oversight-empty">{t("No proof files are waiting for review.", "لا توجد ملفات إثبات بانتظار المراجعة.")}</p>}</CardContent></Card><Card className="maintainr-oversight-card maintainr-card-enter"><CardHeader><CardTitle className="maintainr-oversight-title">{t("Recent operating history", "سجل العمليات الحديث")}</CardTitle></CardHeader><CardContent className="space-y-3">{(oversight.data?.history ?? []).slice(0, 4).map((item) => <div key={item.id} className="maintainr-oversight-item"><div className="maintainr-oversight-item-title">{item.action.replace("_", " ")}</div><div className="maintainr-oversight-item-meta mt-1">{item.message || t("Recorded maintenance activity", "نشاط صيانة مسجل")}</div></div>)}</CardContent></Card><Card className="maintainr-oversight-card maintainr-card-enter"><CardHeader><CardTitle className="maintainr-oversight-title">{t("People and contact records", "سجلات الأشخاص والاتصال")}</CardTitle></CardHeader><CardContent className="space-y-3">{(oversight.data?.contacts ?? []).slice(0, 4).map((person) => <div key={person.id} className="maintainr-oversight-item"><div className="maintainr-oversight-item-title">{person.name || person.email}</div><div className="maintainr-oversight-item-meta mt-1">{person.role} · {person.phone || t("No phone recorded", "لا يوجد رقم هاتف مسجل")}</div></div>)}</CardContent></Card></div>}
  </>;
}
