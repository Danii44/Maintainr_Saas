...
function MessagesPanel() {
  ...
  return <>
    ...
    <div className="grid gap-6 xl:grid-cols-[.75fr_1.45fr]">
      ...
      <Card className="flex min-h-[32rem] flex-col border-[#dce9e6] bg-white"><CardHeader className="border-b border-slate-100"><CardTitle className="text-base text-slate-900">{selected?.subject ?? t("Select a conversation", "اختر محادثة")}</CardTitle><p className="text-xs text-slate-500">{selected ? selected.kind.replace("_", " ") : t("Messages are recorded in the workspace history.", "تسجل الرسائل في سجل مساحة العمل.")}</p></CardHeader><CardContent className="flex flex-1 flex-col p-5"><div className="flex-1 space-y-3 overflow-y-auto">{thread.isLoading ? <p className="text-sm text-slate-400">{t("Loading messages…", "جارٍ تحميل الرسائل…")}</p> : (thread.data?.messages ?? []).length ? thread.data?.messages.map((message) => <div key={message.id} className="max-w-[82%] rounded-2xl bg-slate-100 px-4 py-3 text-sm leading-6 text-slate-700"><div>{message.body}</div><div className="mt-1 text-[10px] text-slate-400">{new Date(message.createdAt).toLocaleString()}</div></div>) : <div className="grid h-full place-items-center text-center text-sm text-slate-400">{t("Choose a conversation or start one with a relevant workspace contact.", "اختر محادثة أو ابدأ واحدة مع جهة اتصال مناسبة في مساحة العمل.")}</div>}</div><div className="mt-5 flex gap-2 border-t border-slate-100 pt-4"><Textarea value={body} onChange={(event) => setBody(event.target.value)} disabled={!selectedId} placeholder={t("Write a message…", "اكتب رسالة…")} className="min-h-12 border-slate-200 bg-slate-50 text-slate-900"/><Button disabled={!selectedId || send.isPending || !body.trim()} className="bg-teal-700 hover:bg-teal-800" onClick={async () => { if (!selectedId) return; try { await send.mutateAsync({ conversationId: selectedId, body }); setBody(""); await thread.refetch(); await utils.operations.messages.list.invalidate(); } catch (error) { toast.error(error instanceof Error ? error.message : t("Unable to send message", "تعذر إرسال الرسالة")); } }}><SendHorizontal size={17}/></Button></div></CardContent></Card>
    </div>
  </>;
}
